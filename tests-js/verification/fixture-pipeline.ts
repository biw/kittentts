...
import { FixturePhonemizer } from "../../src/core/phonemizer.js";
import {
  buildPipelineFeeds,
  chunkMatchesFixture,
...
export async function verifyFixturePhonemizerPipeline(): Promise<void> {
  ...
  const phonemizer = FixturePhonemizer.fromManifest(manifest);
  ...
  assert(actual.num_samples === expected.num_samples, "sample count mismatch for pipeline verifier");
